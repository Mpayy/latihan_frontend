...
export default function Profile() {
  ...
  const displayUsername =
    username || (authUserInfo ? authUserInfo.username : "myusername");
  ...
  const fetchUserPosts = async () => {
    ...
    try {
      const url = username
        ? `/profile/posts?username=${username}`
        : "/profile/posts";
      const res = await api.get(url);
  ...
  const handleCreateComment = async (e) => {
    ...
    try {
      if (editingComment) {
        ...
        Swal.fire("Berhasil", "Komentar berhasil diupdate", "success");
        setEditingComment(null);
      } else {
        // Mode Create
    ...
    return (
      <>
        <div className="text-center py-5">
          ...
          <p className="mt-2 text-muted">Memuat profile...</p>
        </div>
      </>
    );
    ...
    <div className="profile-page animate__animated animate__fadeIn">
      <div className="glass-card mb-5">
        <div className="glass-card-body d-flex flex-column align-items-center text-center">
          ...
          {user.bio && (
            <p
              className="mb-4 text-secondary mx-auto"
          ...
          <div className="d-flex gap-2 gap-sm-4 mb-4">
            <div className="profile-stat-box text-center flex-grow-1">
              <strong className="d-block h5 mb-0 text-primary">
                {user.posts_count}
              </strong>
              <span className="small text-muted fw-medium">Postingan</span>
            </div>
            <div
              className="profile-stat-box text-center flex-grow-1 cursor-pointer"
            ...
            >
              ...
              <span className="small text-muted fw-medium border-hover-bottom">
                Pengikut
              </span>
            </div>
            <div
          ...
          <div className="w-100 d-flex gap-2 justify-content-center">
            {isMe ? (
              <Button className="rounded-pill px-5" onClick={handleEditOpen}>
                Edit Profil
              </Button>
            ) : (
              <Button
                variant={isFollowing ? "outline-secondary" : "primary"}
                className={isFollowing ? "px-5" : "rounded-pill px-5"}
      ...
      <div className="posts-list mt-4">
        {loadingPosts ? (
          ...
          posts.map((post) => (
            <div
              key={post.id}
              className="post-card mb-3 shadow-sm border-light animate__animated animate__fadeInUp"
            >
              ...
              <div className="post-content px-3 py-2 text-start">
                {post.caption}
              </div>
              ...
              <div className="post-likes px-3 mt-1 small fw-bold text-start">
                {post.likes_count || 0} Suka
              </div>
              <div
                className="post-time px-3 text-muted mt-2 mb-3 text-start"
                style={{ fontSize: "11px" }}
              >
                ...
                >
                  <img
                    src={
                      ...
                      defaultAvatar(comment.user?.username)
                    }
                  ...
                  <div className="flex-grow-1 text-start">
                    ...
                    >
                      <div className="d-flex justify-content-between align-items-center gap-3">
                        ...
                        {(comment.user_id === authUserInfo?.id ||
                          authUserInfo?.is_admin) && (
                      ...
                      {comment.file_url && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="d-flex align-items-center gap-2 mt-2 p-1 bg-light rounded text-dark text-decoration-none border small"
                          style={{ fontSize: "11px" }}
                        >
        ...
        <Modal.Footer className="border-top-0 p-2 bg-white flex-column align-items-stretch">
          ...
          {(commentImagePreview || commentFile) && (
            <div className="p-2 mb-2 bg-light rounded border border-dashed d-flex align-items-center gap-3 position-relative">
              <Button
                ...
                onClick={clearCommentAttachments}
              >
                <MdClose size={12} />
              </Button>
          ...
          >
            ...
            <Form.Control
              ...
              type="text"
              placeholder={
                editingComment ? "Edit komentar..." : "Tulis komentar..."
              }
            ...
            <Button
              ...
              disabled={
                isSubmittingComment ||
                (!commentBody.trim() && !commentImage && !commentFile)
              }
            >
              {isSubmittingComment ? (
                <Spinner size="sm" />
            ...
            <div className="text-center mb-4">
              <div className="position-relative d-inline-block">
                <img
                  src={
                    previewImage ||
                    `https://ui-avatars.com/api/?name=${editForm.name}&background=random`
                  }
                  alt="Preview"
      ...
      <Modal
        ...
        onHide={() => setFollowModal({ ...followModal, show: false })}
        centered
        scrollable
      >
          ...
          <div className="p-3">
            {followList.length > 0
              ? followList.map((u) => (
                  <div
                    key={u.id}
                    className="d-flex align-items-center justify-content-between mb-3 bg-white p-2 rounded shadow-sm border animate__animated animate__fadeIn"
                  >
                    <div
                    ...
                    >
                      <img
                        src={u.profile_url || defaultAvatar(u.username)}
                        className="rounded-circle"
                        width="40"
                        height="40"
                        alt={u.name}
